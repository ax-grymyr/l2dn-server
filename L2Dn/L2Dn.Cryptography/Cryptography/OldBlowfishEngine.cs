...
public sealed class OldBlowfishEngine: ICryptoEngine
{
    ...
    public OldBlowfishEngine(ReadOnlySpan<byte> blowfishKey)
    {
        InitArrays(blowfishKey);
    }

    public int Encode(Span<byte> destination, ReadOnlySpan<byte> source)
    {
        if (source.Length % 8 != 0)
            throw new ArgumentException("Source size must be multiple of 8", nameof(source));

        if (destination.Length < source.Length)
            throw new ArgumentException("Destination size is too small", nameof(destination));

        uint[] pArray = _pArray;
        for (int i = 0; i < source.Length; i += 8)
        {
            uint temp1 = LittleEndianBitConverter.ToUInt32(source[i..]);
            uint temp2 = LittleEndianBitConverter.ToUInt32(source[(i + 4)..]);

            for (int j = 0; j < 16; j++)
            {
                uint x = temp1 ^ pArray[j];
                temp1 = F(x) ^ temp2;
                temp2 = x;
            }

            LittleEndianBitConverter.WriteUInt32(destination[i..], temp2 ^ pArray[17]);
            LittleEndianBitConverter.WriteUInt32(destination[(i + 4)..], temp1 ^ pArray[16]);
        }

        return source.Length;
    }

    public int Decode(Span<byte> destination, ReadOnlySpan<byte> source)
    {
        if (source.Length % 8 != 0)
            throw new ArgumentException("Source size must be multiple of 8", nameof(source));
        ...
        for (int i = 0; i < source.Length; i += 8)
        {
            ...
            LittleEndianBitConverter.WriteUInt32(destination[i..], temp2 ^ pArray[0]);
            LittleEndianBitConverter.WriteUInt32(destination[(i + 4)..], temp1 ^ pArray[1]);
        }

        return source.Length;
    }

    private uint F(uint x) => ((_sBox0[(x >> 24) & 0xFF] + _sBox1[(x >> 16) & 0xFF]) ^ _sBox2[(x >> 8) & 0xFF]) +
                              _sBox3[x & 0xFF];

    private void InitArrays(ReadOnlySpan<byte> key)
    {
        int keyIndex = 0;
        ...
        InitSBox(numArray, _pArray);
        InitSBox(numArray, _sBox0);
        InitSBox(numArray, _sBox1);
        InitSBox(numArray, _sBox2);
        InitSBox(numArray, _sBox3);
    }

    private void InitSBox(Span<byte> bytes, uint[] array)
    {
        for (int i = 0; i < array.Length; i += 2)
        {
            Encode(bytes, bytes);
            array[i] = NativeEndianBitConverter.ToUInt32(bytes);
            array[i + 1] = NativeEndianBitConverter.ToUInt32(bytes[4..]);
        }
    }
}
