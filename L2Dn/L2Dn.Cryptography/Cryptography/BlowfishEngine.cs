...
public sealed class BlowfishEngine: ICryptoEngine
{
    ...
    public BlowfishEngine(ReadOnlySpan<byte> blowfishKey)
    {
    ...
    public int Encode(Span<byte> destination, ReadOnlySpan<byte> source)
    {
        ...
        {
            uint xl = LittleEndianBitConverter.ToUInt32(source[i..]);
            uint xr = LittleEndianBitConverter.ToUInt32(source[(i + 4)..]);
            xl ^= p[0];
            xr ^= F(xl) ^ p[1];
            xl ^= F(xr) ^ p[2];
            xr ^= F(xl) ^ p[3];
            ...
            xl ^= F(xr) ^ p[16];
            xr ^= p[17];
            LittleEndianBitConverter.WriteUInt32(destination[i..], xr);
            LittleEndianBitConverter.WriteUInt32(destination[(i + 4)..], xl);
        }
    ...
    private uint F(uint x) => ((_sBox0[x >> 24] + _sBox1[(x >> 16) & 0xFF]) ^ _sBox2[(x >> 8) & 0xFF]) +
                              _sBox3[x & 0xFF];
    ...
    private void InitArrays(ReadOnlySpan<byte> key)
    {
        ...
        ProcessTable(0, 0, _pArray);
        ProcessTable(_pArray[pSize - 2], _pArray[pSize - 1], _sBox0);
        ProcessTable(_sBox0[sSize - 2], _sBox0[sSize - 1], _sBox1);
        ProcessTable(_sBox1[sSize - 2], _sBox1[sSize - 1], _sBox2);
        ProcessTable(_sBox2[sSize - 2], _sBox2[sSize - 1], _sBox3);
    }

    private void ProcessTable(uint xl, uint xr, Span<uint> table)
    {
        uint[] p = _pArray;
