 ...
 * </ul>
 */
public abstract class OldItemTemplate: IIdentifiable, IEventContainerProvider
{
    protected static readonly Logger LOGGER = LogManager.GetLogger(nameof(OldItemTemplate));
    ...
    private Map<Stat, StatFuncParameters>? _funcTemplates;
     ...
     */
    protected OldItemTemplate(StatSet set)
    {
        ...
        _bodyPart = ItemData.SlotNameMap.GetValueOrDefault(set.getString("bodypart", "none"));
     ...
     */
    public void addFunctionTemplate(StatFuncParameters parameters)
    {
        switch (parameters.Stat)
        {
            case Stat.FIRE_RES:
            case Stat.FIRE_POWER:
            {
                setAttributes(new AttributeHolder(AttributeType.FIRE, (int)parameters.Value));
                break;
            }
            case Stat.WATER_RES:
            case Stat.WATER_POWER:
            {
                setAttributes(new AttributeHolder(AttributeType.WATER, (int)parameters.Value));
                break;
            }
            case Stat.WIND_RES:
            case Stat.WIND_POWER:
            {
                setAttributes(new AttributeHolder(AttributeType.WIND, (int)parameters.Value));
                break;
            }
            case Stat.EARTH_RES:
            case Stat.EARTH_POWER:
            {
                setAttributes(new AttributeHolder(AttributeType.EARTH, (int)parameters.Value));
                break;
            }
            case Stat.HOLY_RES:
            case Stat.HOLY_POWER:
            {
                setAttributes(new AttributeHolder(AttributeType.HOLY, (int)parameters.Value));
                break;
            }
            case Stat.DARK_RES:
            case Stat.DARK_POWER:
            {
                setAttributes(new AttributeHolder(AttributeType.DARK, (int)parameters.Value));
                break;
            }
        }
        ...
        if (_funcTemplates.put(parameters.Stat, parameters) != null)
        {
            LOGGER.Warn("Item with id " + _itemId + " has 2 func templates with same stat: " + parameters.Stat);
        }
    }
    ...
    public double getStats(Stat stat, double defaultValue)
    {
        if (_funcTemplates != null)
        {
            StatFuncParameters? template = _funcTemplates.get(stat);
            if (template is { FuncType: StatFuncType.ADD or StatFuncType.SET })
            {
