 ...
 * @author durgus
 */
public class ChatPartyRoomCommander: IChatHandler
{
	...
	public void handleChat(ChatType type, Player activeChar, string target, string text, bool shareLocation)
    {
		...
		if (activeChar.isInParty() && party != null && party.isInCommandChannel() && commandChannel != null &&
            commandChannel.getLeader().Equals(activeChar))
		{
