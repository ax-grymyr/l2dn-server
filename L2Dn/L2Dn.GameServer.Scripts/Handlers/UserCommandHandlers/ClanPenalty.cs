...
{
    private static readonly int[] COMMAND_IDS = [100];
    ...
    public bool useUserCommand(int id, Player player)
    {
        ...
        if (clan != null && clan.getCharPenaltyExpiryTime() > DateTime.UtcNow)
        {
            htmlContent.Append("<td width=170>Unable to invite a clan member.</td><td width=100 align=center>");
            htmlContent.Append(clan.getCharPenaltyExpiryTime()?.ToString("yyyy-MM-dd"));
            htmlContent.Append("</td>");
            penalty = true;
        }
