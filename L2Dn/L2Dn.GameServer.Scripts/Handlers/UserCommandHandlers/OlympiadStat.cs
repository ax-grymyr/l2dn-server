...
{
    private static readonly int[] COMMAND_IDS = [109];
    ...
    public bool useUserCommand(int id, Player player)
    {
        ...
        if (target == null || !target.isPlayer() || targetActingPlayer == null ||
            targetActingPlayer.getClassId().GetLevel() < 2)
        {
        ...
        SystemMessagePacket sm = new SystemMessagePacket(SystemMessageId.
            FOR_THE_CURRENT_OLYMPIAD_YOU_HAVE_PARTICIPATED_IN_S1_MATCH_ES_AND_HAD_S2_WIN_S_AND_S3_DEFEAT_S_YOU_CURRENTLY_HAVE_S4_OLYMPIAD_POINT_S);
        ...
        SystemMessagePacket sm2 =
            new SystemMessagePacket(SystemMessageId.THIS_WEEK_YOU_CAN_PARTICIPATE_IN_A_TOTAL_OF_S1_MATCHES);
