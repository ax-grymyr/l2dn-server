	...
	public override void instant(Creature effector, Creature effected, Skill skill, Item item)
	{
		...
		if (effector.isPlayer() && player != null)
		{
			if (skill.getMaxLightSoulConsumeCount() > 0)
			{
				...
				int chargedSouls = player.getChargedSouls(SoulType.LIGHT) <= skill.getMaxLightSoulConsumeCount() ? player.getChargedSouls(SoulType.LIGHT) : skill.getMaxLightSoulConsumeCount();
				damage *= 1 + chargedSouls * 0.04;
			}
			if (skill.getMaxShadowSoulConsumeCount() > 0)
			{
				// Souls Formula (each soul increase +4%)
				int chargedSouls = player.getChargedSouls(SoulType.SHADOW) <= skill.getMaxShadowSoulConsumeCount() ? player.getChargedSouls(SoulType.SHADOW) : skill.getMaxShadowSoulConsumeCount();
				damage *= 1 + chargedSouls * 0.04;
			}
		}
