...
{
    private static readonly Logger _logger = LogManager.GetLogger(nameof(AdminInstanceZone));
	...
	public bool useAdminCommand(string command, Player activeChar)
    {
		...
		string target = activeCharTarget != null ? activeCharTarget.getName() : "no-target";
			...
			try
			{
                ...
                if (player == null)
                {
                    BuilderUtil.sendSysMessage(activeChar,
                        $"Failed clearing instance time. Player {playerName} not found");
        ...
        if (command.startsWith("admin_instancezone"))
        {
            ...
            else if (activeCharTarget != null)
            {
                if (activeCharTarget.isPlayer())
                {
                    display((Player) activeCharTarget, activeChar);
                }
            }
            else
            {
