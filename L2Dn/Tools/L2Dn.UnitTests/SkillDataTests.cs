    ...
    public void CompareSkills()
    {
        ...
        List<XmlSkill> xmlSkills = Directory.
            ...
            SelectMany(file => XDocument.Load(file).Elements("list").Elements("skill")).Select(el =>
                new XmlSkill((int)(el.Attribute("id") ?? throw new InvalidOperationException("Missing id attribute")),
                    (int)(el.Attribute("toLevel") ?? throw new InvalidOperationException("Missing toLevel attribute")),
                    ((string)(el.Attribute("name") ?? throw new InvalidOperationException("Missing name attribute"))).
                    Trim())).OrderBy(x => x.Id).
            ToList();
