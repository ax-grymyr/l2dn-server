...
public class CursedWeaponsManager: DataReaderBase
{
	...
	private void restore()
	{
		try
		{
			...
			{
				CursedWeapon? cw = _cursedWeapons.get(record.ItemId);
                if (cw == null)
                {
                    LOGGER.Error("CurseWeaponManager: Unknown cursed weapon id=" + record.ItemId);
                    continue;
                }
	...
	public void activate(Player player, Item item)
	{
		...
		if (player.isCursedWeaponEquipped()) // cannot own 2 cursed swords
		{
            ...
            if (cw2 == null)
            {
                LOGGER.Error("Unknown cursed weapon id=" + player.getCursedWeaponEquippedId());
                return;
            }
	...
	public void drop(int itemId, Creature killer)
	{
        ...
        if (cw == null)
        {
            LOGGER.Error("CurseWeaponManager: Unknown cursed weapon id=" + itemId);
            return;
        }
		cw.dropIt(killer);
	}
	...
	public void checkPlayer(Player player)
	{
		...
		foreach (CursedWeapon cw in _cursedWeapons.Values)
		{
            ...
            {
                Item? cwItem = player.getInventory().getItemByItemId(cw.getItemId());
                if (cwItem == null)
                {
                    LOGGER.Error("Item not found in inventory for player " + player.ObjectId + " and cursed weapon " + cw.getItemId());
                    continue;
                }
