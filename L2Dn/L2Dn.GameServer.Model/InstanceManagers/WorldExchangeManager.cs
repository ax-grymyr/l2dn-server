...
public class WorldExchangeManager: DataReaderBase
{
	...
	public WorldExchangeManager()
	{
		...
		{
			TimeSpan interval = TimeSpan.FromMilliseconds(Config.WorldExchange.WORLD_EXCHANGE_SAVE_INTERVAL);
	...
	private void checkBidStatus()
	{
		...
		foreach (var entry in _itemBids)
		{
			...
			switch (holder.getStoreType())
			{
				...
				{
					holder.setEndTime(calculateDate(Config.WorldExchange.WORLD_EXCHANGE_ITEM_BACK_PERIOD));
				...
				case WorldExchangeItemStatusType.WORLD_EXCHANGE_OUT_TIME:
				{
					...
					item.updateDatabase(!Config.WorldExchange.WORLD_EXCHANGE_LAZY_UPDATE);
					break;
				}
			}
		}
		...
		try
		{
			...
			foreach (DbWorldExchangeItem record in ctx.WorldExchangeItems)
			{
				...
				if (endTime < DateTime.UtcNow)
				{
					...
					endTime = calculateDate(Config.WorldExchange.WORLD_EXCHANGE_ITEM_BACK_PERIOD);
	...
	private long calculateFeeForRegister(Player player, int objectId, long amount, long priceForEach)
	{
        ...
        {
            LOGGER.Error("Item not found in inventory for objectId: " + objectId);
            return 0;
        }
	...
	public void registerItemBid(Player player, int itemObjectId, long amount, long priceForEach)
	{
		...
		if (Config.WorldExchange.WORLD_EXCHANGE_MAX_ADENA_FEE != -1 && feePrice > Config.WorldExchange.WORLD_EXCHANGE_MAX_ADENA_FEE)
		{
			feePrice = Config.WorldExchange.WORLD_EXCHANGE_MAX_ADENA_FEE;
		}
        ...
        if (item == null)
        {
            LOGGER.Error("Item not found in inventory for objectId: " + itemObjectId);
            player.sendPacket(new SystemMessagePacket(SystemMessageId.THE_ITEM_IS_NOT_FOUND));
            player.sendPacket(WorldExchangeRegisterItemPacket.FAIL);
            return;
        }
		...
		if (item.isStackable() && player.getInventory().getInventoryItemCount(item.Id, -1) > amount)
		{
        ...
        if (!_itemCategories.TryGetValue(itemInstance.Id, out WorldExchangeItemSubType category))
		{
		...
		DateTime endTime = calculateDate(Config.WorldExchange.WORLD_EXCHANGE_ITEM_SELL_PERIOD);
	...
	private void takeBidMoney(Player player, WorldExchangeHolder worldExchangeItem)
	{
		...
		player.sendPacket(new WorldExchangeSettleRecvResultPacket(worldExchangeItem.getItemInstance().ObjectId, worldExchangeItem.getItemInstance().getCount(), (byte) 1));
		long fee = (long)(worldExchangeItem.getPrice() * Config.WorldExchange.WORLD_EXCHANGE_LCOIN_TAX * 100 / 100);
		long returnPrice = worldExchangeItem.getPrice() - Math.Min(fee, Config.WorldExchange.WORLD_EXCHANGE_MAX_LCOIN_TAX != -1 ? Config.WorldExchange.WORLD_EXCHANGE_MAX_LCOIN_TAX : long.MaxValue); // floating-point accuracy workaround :D
	...
	public void buyItem(Player player, int worldExchangeId)
	{
		...
		DateTime destroyTime = calculateDate(Config.WorldExchange.WORLD_EXCHANGE_PAYMENT_TAKE_PERIOD);
		...
		{
			if (oldOwner.ObjectId == newHolder.getOldOwnerId())
			{
				oldOwner.sendPacket(new WorldExchangeSellCompleteAlarmPacket(newItem.Id, newItem.getCount()));
				break;
			}
		}
	}
	...
	{
		Item newItem = new Item(oldItem.Id);
		newItem.setOwnerId(requestor.ObjectId);
	...
	public List<WorldExchangeHolder> getItemBids(List<int> ids, WorldExchangeSortType sortType, string lang)
	{
		...
		foreach (WorldExchangeHolder holder in _itemBids.Values)
		{
			...
			if (ids.Contains(holder.getItemInstance().Id) && holder.getStoreType() == WorldExchangeItemStatusType.WORLD_EXCHANGE_REGISTERED)
			{
				returnList.Add(holder);
			}
		}
		...
		switch (sortType)
		{
			...
			case WorldExchangeSortType.ITEM_NAME_ASCE:
			{
				if (lang == null || (!lang.equals("en") && _localItemNames.ContainsKey(lang)))
				{
					...
					sortedList.Sort((a, b) =>
						getItemName(lang ?? "en", a.getItemInstance().Id, a.getItemInstance().isBlessed())
							.CompareTo(getItemName(lang ?? "en", b.getItemInstance().Id,
	...
	public void checkPlayerSellAlarm(Player player)
	{
		...
		foreach (WorldExchangeHolder holder in _itemBids.Values)
		{
			...
			{
				player.sendPacket(new WorldExchangeSellCompleteAlarmPacket(holder.getItemInstance().Id, holder.getItemInstance().getCount()));
				break;
			}
		}
	}
	...
	public void storeMe()
	{
		if (!Config.WorldExchange.ENABLE_WORLD_EXCHANGE || !Config.WorldExchange.WORLD_EXCHANGE_LAZY_UPDATE)
		{
			return;
		}
	...
	public void insert(long worldExchangeId, bool remove)
	{
		...
		try
		{
            ...
            if (holder == null)
            {
                LOGGER.Error("Error while saving World Exchange item bid " + worldExchangeId + " - holder is null");
                return;
            }
	...
	public long getAveragePriceOfItem(int itemId)
	{
		...
		foreach (WorldExchangeHolder holder in _itemBids.Values)
		{
			if (holder.getItemInstance().getTemplate().Id != itemId)
			{
				continue;
			}
