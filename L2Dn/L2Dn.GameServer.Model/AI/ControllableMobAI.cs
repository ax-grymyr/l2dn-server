...
public class ControllableMobAI : AttackableAI
{
	...
	protected void thinkFollow()
	{
		...
		if (!Util.checkIfInRange(MobGroupTable.FOLLOW_RANGE, me, forcedTarget, true))
		{
            ...
            moveTo(new Location3D(forcedTarget.getX() + signX * randX, forcedTarget.getY() + signY * randY,
                forcedTarget.getZ()));
        }
	}
	...
	protected void thinkForceAttack()
    {
		...
		if (!_actor.isMuted() && dist2 > (range + 20) * (range + 20))
		{
			...
			if (!_isNotMoving)
			{
				moveToPawn(forcedTarget, _actor.getPhysicalAttackRange()/* range */);
			}

			return;
		}

		_actor.doAutoAttack(forcedTarget);
	}
