...
public class AttackableAI: CreatureAI
{
	...
	protected override void changeIntention(CtrlIntention newIntention, params Object[] args)
	{
		...
		if ((intention == CtrlIntention.AI_INTENTION_IDLE) || (intention == CtrlIntention.AI_INTENTION_ACTIVE))
		{
			...
			if (!npc.isAlikeDead())
			{
				...
				else if ((npc.getSpawn() != null) && !npc.isInsideRadius3D(npc.getSpawn().Location.Location3D, Config.MAX_DRIFT_RANGE + Config.MAX_DRIFT_RANGE))
				{
					intention = CtrlIntention.AI_INTENTION_ACTIVE;
				}
			}
	...
	protected virtual void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			if (npc.isFakePlayer() && npc.isAggressive())
			{
				...
				if (droppedItems.isEmpty())
				{
					...
					foreach (Creature t in World.getInstance().getVisibleObjectsInRange<Creature>(npc, npc.getAggroRange()))
					{
							...
							|| (Config.FAKE_PLAYER_AGGRO_PLAYERS && t.isPlayer()))
						{
							...
							double distance = npc.calculateDistance2D(t.getLocation().Location2D);
				...
				else if (!npc.isInCombat()) // must pickup items
				{
					...
					{
						if (npc.calculateDistance2D(droppedItem.getLocation().Location2D) > 50)
						{
		...
		if (!npc.isWalker() && (npc.getSpawn() != null) && (npc.calculateDistance2D(npc.getSpawn().Location.Location2D) > Config.MAX_DRIFT_RANGE) && ((getTarget() == null) || getTarget().isInvisible() || (getTarget().isPlayer() && !Config.ATTACKABLES_CAMP_PLAYER_CORPSES && getTarget().getActingPlayer().isAlikeDead())))
		{
			npc.setWalking();
			npc.returnHome();
			return;
		...
		if ((leader != null) && !leader.isAlikeDead())
		{
			...
			if (npc.calculateDistanceSq2D(leader.getLocation().Location2D) > (offset * offset))
			{
	...
	protected virtual void thinkAttack()
	{
		...
		if (Config.AGGRO_DISTANCE_CHECK_ENABLED && npc.isMonster() && !npc.isWalker() && !(npc is GrandBoss))
		{
			...
			if ((spawn != null) && (npc.calculateDistance2D(spawn.Location.Location2D) > (spawn.getChaseRange() > 0 ? Math.Max(Config.MAX_DRIFT_RANGE, spawn.getChaseRange()) : npc.isRaid() ? Config.AGGRO_DISTANCE_CHECK_RAID_RANGE : Config.AGGRO_DISTANCE_CHECK_RANGE)))
			{
		...
		if (!npc.isMovementDisabled() && (Rnd.get(100) <= 3))
		{
			...
			{
				if (npc.isInsideRadius2D(nearby.getLocation().Location2D, collision) && (nearby != target))
				{
		...
		{
			double distance2 = npc.calculateDistanceSq2D(target.getLocation().Location2D);
		...
		if ((!npc.isMoving() && npc.hasSkillChance()) || (npc.getAiType() == AIType.MAGE))
		{
			...
			if (!npc.getShortRangeSkills().isEmpty() && (npc.calculateDistance2D(target.getLocation().Location2D) <= 150))
			{
		...
		if (npc.calculateDistance2D(target.getLocation().Location2D) > range)
		{
	...
	private bool checkTarget(WorldObject target)
	{
		...
		if (target.isCreature())
		{
			...
			{
				if (!npc.isInsideRadius2D(target.getLocation().Location2D, npc.getPhysicalAttackRange() + npc.getTemplate().getCollisionRadius() + ((Creature) target).getTemplate().getCollisionRadius()))
				{
					return false;
				}
