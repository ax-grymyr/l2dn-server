    ...
    public ValueTask ProcessAsync(Connection connection, GameSession session)
    {
        ...
        if (player.isMentor())
        {
            ...
            if (mentee != null)
            {
                ...
                SystemMessagePacket sm = new SystemMessagePacket(SystemMessageId.
                    S1_S_MENTORING_CONTRACT_IS_CANCELLED_THE_MENTOR_CANNOT_BOND_WITH_ANOTHER_MENTEE_FOR_2_DAYS);

                sm.Params.addString(_name);
                player.sendPacket(sm);

                MentorManager.getInstance().setPenalty(player.ObjectId,
                    TimeSpan.FromDays(Config.Character.MENTOR_PENALTY_FOR_MENTEE_LEAVE)); // TODO: verify period
        ...
        else if (player.isMentee())
        {
            ...
            if (mentor != null && mentor.getObjectId() == objectId)
            {
                ...
                MentorManager.getInstance().setPenalty(mentor.getObjectId(),
                    TimeSpan.FromDays(Config.Character.MENTOR_PENALTY_FOR_MENTEE_LEAVE)); // TODO: verify period
