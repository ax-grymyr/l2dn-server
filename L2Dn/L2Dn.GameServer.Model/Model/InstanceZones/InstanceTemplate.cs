...
public class InstanceTemplate: IIdentifiable, INamable, IEventContainerProvider
{
	...
	private TimeSpan _ejectTime = TimeSpan.FromMinutes(Config.General.EJECT_DEAD_PLAYER_TIME);
	private readonly int _maxWorldCount;
	private bool _isPvP;
	private bool _allowPlayerSummon;
	private float _expRate = Config.Rates.RATE_INSTANCE_XP;
	private float _spRate = Config.Rates.RATE_INSTANCE_SP;
	private float _expPartyRate = Config.Rates.RATE_INSTANCE_PARTY_XP;
	private float _spPartyRate = Config.Rates.RATE_INSTANCE_PARTY_SP;
	...
	public Location3D? getExitLocation(Player player)
	{
		...
		switch (_exitLocationType)
		{
			...
			case InstanceTeleportType.TOWN:
			{
				...
				{
					location = MapRegionManager.GetNearestKarmaRespawn(player);
				}
				else
				{
					location = MapRegionManager.GetNearestTownRespawn(player);
				}
				break;
			}
		}
