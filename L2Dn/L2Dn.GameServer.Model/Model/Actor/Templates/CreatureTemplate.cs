...
/// Creature template.
/// </summary>
public class CreatureTemplate: IEventContainerProvider
{
    private readonly EventContainer _eventContainer;

    // For client info use _fCollisionRadius
    private readonly int _collisionRadius;

    // For client info use _fCollisionHeight
    private readonly int _collisionHeight;

    private readonly float _fCollisionRadius;
    private readonly float _fCollisionHeight;

    // BaseStats
    private readonly Map<Stat, double> _baseValues = [];
    ...
    public CreatureTemplate(StatSet set)
    {
        ...
        // Base stats
        _baseValues[Stat.STAT_STR] = set.getDouble("baseSTR", 0);
        _baseValues[Stat.STAT_CON] = set.getDouble("baseCON", 0);
        _baseValues[Stat.STAT_DEX] = set.getDouble("baseDEX", 0);
        _baseValues[Stat.STAT_INT] = set.getDouble("baseINT", 0);
        _baseValues[Stat.STAT_WIT] = set.getDouble("baseWIT", 0);
        _baseValues[Stat.STAT_MEN] = set.getDouble("baseMEN", 0);

        // Max HP/MP/CP
        _baseValues[Stat.MAX_HP] = set.getDouble("baseHpMax", 0);
        _baseValues[Stat.MAX_MP] = set.getDouble("baseMpMax", 0);
        _baseValues[Stat.MAX_CP] = set.getDouble("baseCpMax", 0);

        // Regenerate HP/MP/CP
        _baseValues[Stat.REGENERATE_HP_RATE] = set.getDouble("baseHpReg", 0);
        _baseValues[Stat.REGENERATE_MP_RATE] = set.getDouble("baseMpReg", 0);
        _baseValues[Stat.REGENERATE_CP_RATE] = set.getDouble("baseCpReg", 0);

        // Attack and Defense
        _baseValues[Stat.PHYSICAL_ATTACK] = set.getDouble("basePAtk", 0);
        _baseValues[Stat.MAGIC_ATTACK] = set.getDouble("baseMAtk", 0);
        _baseValues[Stat.PHYSICAL_DEFENCE] = set.getDouble("basePDef", 0);
        _baseValues[Stat.MAGICAL_DEFENCE] = set.getDouble("baseMDef", 0);

        // Attack speed
        _baseValues[Stat.PHYSICAL_ATTACK_SPEED] = set.getDouble("basePAtkSpd", 300);
        _baseValues[Stat.MAGIC_ATTACK_SPEED] = set.getDouble("baseMAtkSpd", 333);

        // Misc
        _baseValues[Stat.SHIELD_DEFENCE] = set.getDouble("baseShldDef", 0);
        _baseValues[Stat.PHYSICAL_ATTACK_RANGE] = set.getDouble("baseAtkRange", 40);
        _baseValues[Stat.RANDOM_DAMAGE] = set.getDouble("baseRndDam", 0);

        // Shield and critical rates
        _baseValues[Stat.SHIELD_DEFENCE_RATE] = set.getDouble("baseShldRate", 0);
        _baseValues[Stat.CRITICAL_RATE] = set.getDouble("baseCritRate", 4);
        _baseValues[Stat.MAGIC_CRITICAL_RATE] = set.getDouble("baseMCritRate", 5);

        // Breath under water
        _baseValues[Stat.BREATH] = set.getDouble("baseBreath", 100);

        // Elemental Attributes
        // Attack
        _baseValues[Stat.FIRE_POWER] = set.getDouble("baseFire", 0);
        _baseValues[Stat.WIND_POWER] = set.getDouble("baseWind", 0);
        _baseValues[Stat.WATER_POWER] = set.getDouble("baseWater", 0);
        _baseValues[Stat.EARTH_POWER] = set.getDouble("baseEarth", 0);
        _baseValues[Stat.HOLY_POWER] = set.getDouble("baseHoly", 0);
        _baseValues[Stat.DARK_POWER] = set.getDouble("baseDark", 0);

        // Defense
        _baseValues[Stat.FIRE_RES] = set.getDouble("baseFireRes", 0);
        _baseValues[Stat.WIND_RES] = set.getDouble("baseWindRes", 0);
        _baseValues[Stat.WATER_RES] = set.getDouble("baseWaterRes", 0);
        _baseValues[Stat.EARTH_RES] = set.getDouble("baseEarthRes", 0);
        _baseValues[Stat.HOLY_RES] = set.getDouble("baseHolyRes", 0);
        _baseValues[Stat.DARK_RES] = set.getDouble("baseDarkRes", 0);
        _baseValues[Stat.BASE_ATTRIBUTE_RES] = set.getDouble("baseElementRes", 0);
        ...
        // Speed
        _baseValues[Stat.RUN_SPEED] = set.getDouble("baseRunSpd", 120);
        _baseValues[Stat.WALK_SPEED] = set.getDouble("baseWalkSpd", 50);

        // Swimming
        _baseValues[Stat.SWIM_RUN_SPEED] = set.getDouble("baseSwimRunSpd", 120);
        _baseValues[Stat.SWIM_WALK_SPEED] = set.getDouble("baseSwimWalkSpd", 50);

        // Flying
        _baseValues[Stat.FLY_RUN_SPEED] = set.getDouble("baseFlyRunSpd", 120);
        _baseValues[Stat.FLY_WALK_SPEED] = set.getDouble("baseFlyWalkSpd", 50);

        // Attack type
        _baseAttackType = set.getEnum("baseAtkType", WeaponType.FIST);

        // Race
        _race = set.getEnum("race", Race.NONE);

        // Basic property
        _baseValues[Stat.ABNORMAL_RESIST_PHYSICAL] = set.getDouble("physicalAbnormalResist", 10);
        _baseValues[Stat.ABNORMAL_RESIST_MAGICAL] = set.getDouble("magicAbnormalResist", 10);
    }
