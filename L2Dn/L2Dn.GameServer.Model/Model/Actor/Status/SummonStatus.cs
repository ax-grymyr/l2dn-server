...
public class SummonStatus: PlayableStatus
{
    ...
    public override void reduceHp(double amount, Creature? attacker, bool awake, bool isDOT, bool isHPConsumption)
    {
        ...
        if (party != null)
        {
            ...
            if (caster != null && Util.checkIfInRange(1000, getActiveChar(), caster, true) && !caster.isDead() &&
                party.getMembers().Contains(caster) && casterParty != null)
            {
                int transferDmg = (int)value *
                    (int)getActiveChar().getStat().getValue(Stat.TRANSFER_DAMAGE_TO_PLAYER, 0) / 100;
                ...
                if (transferDmg > 0)
                {
                    ...
                    if (membersInRange > 0)
                    {
                        caster.reduceCurrentHp(1.0 * transferDmg / membersInRange, attacker, null);
                        value -= transferDmg;
                    }
                }
            }
        ...
        {
            int transferDmg = (int)value * (int)getActiveChar().getStat().getValue(Stat.TRANSFER_DAMAGE_TO_PLAYER, 0) /
                100;
