...
public class CreatureStat
{
	...
	private readonly Set<TraitType> _attackTraits = [];
	private readonly Set<TraitType> _defenceTraits = [];
	private readonly Set<TraitType> _invulnerableTraits = [];
	...
	public virtual void recalculateStats(bool broadcast)
	{
		...
		lock (_lock)
		{
			...
			foreach (StatHolder holder in _additionalAdd)
			{
				...
				{
					mergeAdd(holder.Stat, holder.Value);
				}
			}
			foreach (StatHolder holder in _additionalMul)
			{
				if (holder.VerifyCondition(_creature))
				{
					mergeMul(holder.Stat, holder.Value);
				}
			}
	...
	public bool removeAddAdditionalStat(Stat stat, double value)
	{
		...
		{
			if (holder.Stat == stat && holder.Value == value)
			{
