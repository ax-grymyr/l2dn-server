...
{
    private static readonly Logger _logger = LogManager.GetLogger(nameof(World));
    ...
    public void addObject(WorldObject @object)
    {
        ...
        if (@object.isPlayer())
        {
            ...
            if (existingPlayer != newPlayer)
            {
                ...
                _logger.Warn(GetType().Name + ": Duplicate character!? Disconnected both characters (" +
                    newPlayer.getName() + ")");
            }
     ...
     */
    public int getVisibleObjectsCount() => _allObjects.Count;

    public ICollection<Player> getPlayers() => _allPlayers.Values;

    public ICollection<Player> getAllGoodPlayers() => _allGoodPlayers.Values;

    public ICollection<Player> getAllEvilPlayers() => _allEvilPlayers.Values;
    ...
    public ICollection<Player> getSellingOrBuyingPlayers()
    {
        lock (_allStoreModeBuySellPlayers)
        {
            ...
            {
                _nextPrivateStoreUpdate =
                    currentTime + TimeSpan.FromMilliseconds(Config.General.STORE_REVIEW_CACHE_TIME);
    ...
    public void disposeOutOfBoundsObject(WorldObject @object)
    {
        ...
        else if (_allObjects.remove(@object.ObjectId) != null)
        {
            ...
            {
                Npc npc = (Npc)@object;
                _logger.Warn("Deleting npc " + @object.getName() + " NPCID[" + npc.Id + "] from invalid location X:" +
                    @object.getX() + " Y:" + @object.getY() + " Z:" + @object.getZ());
                ...
                if (spawn != null)
                {
                    _logger.Warn("Spawn location: " + spawn.Location);
                }
            }
            else if (@object.isCreature())
            {
                _logger.Warn("Deleting object " + @object.getName() + " OID[" + @object.ObjectId +
                    "] from invalid location X:" + @object.getX() + " Y:" + @object.getY() + " Z:" + @object.getZ());
    ...
    public int getPartyCount() => _partyNumber;

    public int getPartyMemberCount() => _memberInPartyNumber;
