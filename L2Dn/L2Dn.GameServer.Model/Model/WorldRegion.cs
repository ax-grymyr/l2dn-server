...
public class WorldRegion
{
	...
	private readonly Set<WorldObject> _visibleObjects = new();
	...
	private WorldRegion[] _surroundingRegions = Array.Empty<WorldRegion>();
		...
		if (!isOn)
		{
			foreach (WorldObject wo in _visibleObjects)
			{
				if (wo.isAttackable())
				{
					...
					if ((spawn != null) && (mob.calculateDistance2D(spawn.Location.Location.ToLocation2D()) > Config.MAX_DRIFT_RANGE))
					{
